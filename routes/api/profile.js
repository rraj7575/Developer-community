...
router.get("/handle/:handle", (req, res) => {
  ...
  Profile.findOne({ handle: req.params.handle })
    ...
    .then((profile) => {
      if (!profile) {
        errors.nohandle = "There is no profile for this handle";
        return res.status(404).json(errors);
      }
      res.json(profile);
    })
...
//@route POST api/profile/user/all
...
router.get("/all", (req, res) => {
  ...
  Profile.find()
    ...
    .catch((err) => res.json({ profile: "Thre is no profile" }));
});
...
//@desc experience to user profile
//@access Private
...
//@desc education to user profile
//@access Private
...
router.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      ...
      .catch((ere) => res.status(404).json(ere));
  }
);
